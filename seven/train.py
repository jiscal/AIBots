...
def setup_train_args():
    ...
    parser.add_argument('--raw', action='store_true', default=True,help='是否对原始训练语料做tokenize。若尚未对原始训练语料进行tokenize，则指定该参数')
                    ...
                    if (overall_step + 1) % args.log_step == 0:
                        logger.info(
                            "batch {} of epoch {}, loss {}, accuracy {}".format(batch_idx + 1, epoch + 1, loss,
                                                                                accuracy))
                        print("batch {} of epoch {}, loss {}, accuracy {}".format(batch_idx + 1, epoch + 1, loss,
                                                                                accuracy))
...
def evaluate(model, device, test_list, multi_gpu, args):
    ...
    test_dataloader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers,
                                 collate_fn=collate_fn)
    with torch.no_grad():
        for batch_idx, input_idss in enumerate(test_dataloader):
            input_idss=input_idss.to(device)
            outputs = model.forward(input_ids=input_idss)
            loss, accuracy = calculate_loss_and_accuracy(outputs, labels=input_idss, device=device)
